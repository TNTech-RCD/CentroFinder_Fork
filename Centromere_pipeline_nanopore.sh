...
WINDOW=1000
ASSEMBLY=Guy11_Final_S4.fasta
TRF=Guy11_trf.bed
TE=Guy11_edta.bed
HIFI=bed_meth/Guy11_sorted.bam
METH=bed_meth/Guy11.bed
GENES=Guy11.gff3    # optional
...
# Create genome windows, sort, and save
bedtools makewindows -g ${ASSEMBLY}.fai -w $WINDOW \
    | sort -k1,1V -k2,2n > $OUTDIR/windows.${WINDOW}bp.bed
...
awk 'BEGIN{OFS="\t"} !/^#/ && $5!="nan" && $5!="NA" && $5!="" {
    ...
    print $1, $2, $3, val
}' $METH | sort -k1,1 -k2,2n > $OUTDIR/methylation.sorted.bedgraph
...
samtools depth -a $HIFI | awk '{print $1"\t"$2-1"\t"$2"\t"$3}' \
    | sort -k1,1V -k2,2n > $OUTDIR/hifi.depth.sorted.bed # <-- NEW: sort and rename to .sorted
    
# Use the sorted file
bedtools map -a $OUTDIR/windows.${WINDOW}bp.bed -b $OUTDIR/hifi.depth.sorted.bed -c 4 -o mean -null 0 > $OUTDIR/tmp.hifi_cov_mean.bed
...
bedtools map -a $OUTDIR/windows.${WINDOW}bp.bed -b $OUTDIR/methylation.sorted.bedgraph -c 4 -o mean -null 0 > $OUTDIR/tmp.meth_mean.bed
...
for chrom, sub in df.groupby('chrom'):
    ...
    if internal_perfect_sub.empty:
        print(f"Warning: No internal windows with zero TE/Gene found on {chrom}. Selecting best score as fallback.")
...
# ==============================
# Plotting and gene-free region identification
# ==============================
...
for chrom, g in df.groupby('chrom'):
    ...
    if chrom_candidates.empty:
        print(f"Skipping {chrom}: No valid centromere candidates found.")
        continue

    idx = chrom_candidates['centro_score'].idxmax()
    if pd.isna(idx):
        print(f"Warning: idxmax() returned NaN for {chrom}. Skipping.")
        continue

    best_window = chrom_candidates.loc[idx]
