...
for chrom, g in df.groupby('chrom'):
    ...
    # ---- Top panel: features ----
    line1, = ax1.plot(x, g['trf_cov_n'], lw=1, label='TRF')
    # Plot the modified, penalized TE score
    line2, = ax1.plot(x, g['te_cov_n'], lw=1, label='TE')
    line3, = ax1.plot(x, g['meth_diff_n'], lw=1, label='Meth')
    line4, = ax1.plot(x, g['gene_count_n'], lw=1, label='Gene')
    line5, = ax1.plot(x, g['gc_low_n'], lw=1, label='GC', color='purple')
    ax1.set_ylabel("Normalized Feature Values")
    ax1.set_title(f"{chrom} - Features & Centromere Score") # (Subtelomere Penalized)

    # ---- Bottom panel: centromere score ----
    line6, = ax2.plot(x, g['centro_score'], color='black', lw=1.5, label='Centromere Score')
    ax2.set_xlabel(f"{chrom} position (kb)")
    ax2.set_ylabel("Centromere Score")

    # ---- Highlight gene-free region (now CONTIGUOUS) ----
    if no_gene_region is not None:
        ax2.axvspan(no_gene_region[0], no_gene_region[1], color='yellow', alpha=0.3,
                     label='Centromere Region')

    # ---- Legends outside ----
    handles = [line1, line2, line3, line4, line5, line6]
    if no_gene_region is not None:
        # Add a placeholder for the axvspan if it's present, so it gets a legend entry
...
# Save best windows per chromosome
...
# Save gene-free regions to a new BED file (NOW CONTIGUOUS)
if gene_free_regions:
    gene_free_df = pd.DataFrame(gene_free_regions, columns=['chrom', 'start', 'end', 'name'])
    # Write the BED file (chrom, start, end, name)
    gene_free_df.to_csv(out_prefix + "_best_candidates.bed", sep='\t', index=False, header=False)
else:
    print("Warning: No strict contiguous gene-free/low-TE regions identified around best centromere candidates to save.")
